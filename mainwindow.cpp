...
*/
    ...
    workBox = new QTextEdit(tr("<i>Для загрузки файла выберете соответствующий пункт в меню Файл</i>"));
    workBox->setFrameStyle(QFrame::StyledPanel | QFrame::Sunken);
    workBox->setAlignment(Qt::AlignCenter);
/*
...
//! [2]
    ...
    QString message = tr("Контекстное меню доступно нажатием правой кнопкой мыши");
    statusBar()->showMessage(message);

    setWindowTitle(tr("Приложение для открытия файлов"));
...
void MainWindow::open()
{
    ...
    if (!file1.exists())
    {
        qWarning("Файл не существует"); // если файл не найден, то выводим предупреждение и завершаем работу программы
    }

    // С помощью метода open() открываем файл в режиме чтения
    if (!file1.open(QIODevice::ReadOnly))
    {
      qWarning("Не получается прочитать файл"); // если файл не найден, то выводим предупреждение и завершаем выполнение программы
    }
    ...
    workBox->setAlignment(Qt::AlignLeft);
...
{
    //workBox->setText(tr("Invoked <b>File|Close</b>"));
    workBox->clear();
    workBox->setText(tr("<i>Файл успешно закрыт</i>"));
    workBox->setAlignment(Qt::AlignCenter);
}
...
*/
//! [4]
    openAct = new QAction(tr("Загрузить файл..."), this);
    openAct->setShortcuts(QKeySequence::Open);
    openAct->setStatusTip(tr("Выберете файл, который хотите открыть"));
    connect(openAct, &QAction::triggered, this, &MainWindow::open);
//! [5]
    closeAct = new QAction(tr("&Закрыть"), this);
    closeAct->setShortcuts(QKeySequence::Close);
    closeAct->setStatusTip(tr("Закройте файл"));
